      ...
      <section>
        <div className="container">
          <div className="row">
              ...
              <a href="https://elmamouzan.ir" className="text-dark"><h4><span className="text-white">دوره</span> های رایگان</h4></a>
              <h5>کسب کار شما با یادگیری پیشرفت میکند...</h5>
              </div>
              <div className="col-md-6 my-4">
                  <Link to="/all-courses"><button className="btn-nav">همه دوره های رایگان</button></Link>
              </div>
              {indexCourses.map(item => {
                if(item.status === "public" && item.price == 0){
                  const base64String = btoa(
                    String.fromCharCode(
                      ...new Uint8Array(item.img.data.data)
                    )
